...
class HomeController extends GetxController {
  ...
  final Map<int, Set<int>> _vendorMenuIds = {};  // vendor_id -> set<linked_menu_item ids>
  ...
  linkedMenuItem: (m['linked_menu_item'] as int), // <-- keep linked menu id if your model supports it
  ...
  if (searchMode.value != SearchMode.none && _lc(currentQuery.value).isNotEmpty) {
  return;
  }
  ...
  Future<bool> runSearch(String query) async {
    ...
    try {
      ...
      for (final n in _allNear)  { vendorNameMap[n.vendorId] = n.name; }
      for (final b in _allBeyond){ if (b.name.isNotEmpty) vendorNameMap[b.userId] = b.name; }
      for (final s in _allSpeedy){ if (s.name.isNotEmpty) vendorNameMap[s.userId] = s.name; }
      ...
      if (vendorMatches.isNotEmpty) {
        ...
        final near   = _allNear.where((e) => vendorMatches.contains(e.vendorId)).toList();
        final beyond = _allBeyond.where((e) => vendorMatches.contains(e.userId)).toList();
        final speedy = _allSpeedy.where((e) => vendorMatches.contains(e.userId)).toList();
      ...
      // 2) Deal title match
      final beyondDealMatches = _allBeyond.where((e) => _contains(_lc(e.offers), q)).toList();
      final speedyDealMatches = _allSpeedy.where((e) => _contains(_lc(e.offers), q)).toList();

      if (beyondDealMatches.isNotEmpty || speedyDealMatches.isNotEmpty) {
        searchMode.value = SearchMode.deal;

        final vendorIds = <int>{};
        for (final e in beyondDealMatches) { vendorIds.add(e.userId); }
        for (final e in speedyDealMatches) { vendorIds.add(e.userId); }

        final near = _allNear.where((e) => vendorIds.contains(e.vendorId)).toList();
      ...
      if (matchedVendorIds.isNotEmpty || matchedMenuIds.isNotEmpty) {
        final near = _allNear.where((e) => matchedVendorIds.contains(e.vendorId)).toList();
  ...
  final headers = await BaseClient.authHeaders();
  final uri = Uri.parse(
  'https://intensely-optimal-unicorn.ngrok-free.app/vendors/deals/?user_id=$vendorId',
  );
  final res = await BaseClient.getRequest(api: uri.toString(), headers: headers);
  if (res.statusCode >= 200 && res.statusCode < 300) {
  ...
  for (final b in _allBeyond)   { s.add(b.userId); }
  for (final sdy in _allSpeedy) { s.add(sdy.userId); }
  for (final n in _allNear)     { s.add(n.vendorId); }
  return s;
  }
