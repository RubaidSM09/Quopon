...
bool isDateMatch(String filter, String dateStr) {
  DateTime orderDate = DateTime.parse(dateStr).toLocal();
  DateTime now = DateTime.now();
  if (filter == 'Today') {
    return orderDate.year == now.year && orderDate.month == now.month && orderDate.day == now.day;
  } else if (filter == 'Yesterday') {
    DateTime yesterday = now.subtract(Duration(days: 1));
    return orderDate.year == yesterday.year && orderDate.month == yesterday.month && orderDate.day == yesterday.day;
  } else if (filter == 'Last Week') {
    DateTime lastWeek = now.subtract(Duration(days: 7));
    return orderDate.isAfter(lastWeek) || orderDate.isAtSameMomentAs(lastWeek);
  } else if (filter == 'Last Month') {
    DateTime lastMonth = now.subtract(Duration(days: 30));
    return orderDate.isAfter(lastMonth) || orderDate.isAtSameMomentAs(lastMonth);
  }
  return false;
}

class DeliveryOrderReceivedView extends GetView<MyOrdersVendorsController> {
  DeliveryOrderReceivedView({super.key});
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      var filtered = controller.orders.where((o) => o.deliveryType == 'DELIVERY' && (o.status == 'PENDING_PAYMENT' || o.status == 'RECEIVED') && isDateMatch(selectedDate.value, o.createdAt)).toList();
      return Column(
        children: [
          Row(
            children: [
